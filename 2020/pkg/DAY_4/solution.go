...
func processData(path string) []map[string]string {
	...
	for _, entry := range raw {
		...
		fields := strings.Split(sanitizedEntry, " ")
		entryMap := map[string]string{}
		for _, field := range fields {
			keyVal := strings.Split(field, ":")
			entryMap[keyVal[0]] = keyVal[1]
		}
...
Thoughts:
	...
	two valid cases; map with all 8 fields AND 7 fields as long as the field that is missing is cid.
	Turn each entry into a map to gurantee constant time check for 7 fields case.
*/
func partOne(entries []map[string]string) int {
	valid := 0
...
func checkNumericField(field string, lower int, upper int) bool {
	val, err := strconv.Atoi(field)
	if (err == nil) && (val >= lower && val <= upper) {
		return true
	}
	return false
}

func partTwo(entries []map[string]string) int {
	valid := 0
	hclPattern, _ := regexp.Compile("[a-f0-9]")
	...
	for _, entry := range entries {
		// minmum req check
		_, ok := entry["cid"]
		if (len(entry) == 8) || (len(entry) == 7 && !ok) {
			// check numeric simple numeric fields byr, iyr, eyr
			if !checkNumericField(entry["byr"], 1920, 2002) ||
				!checkNumericField(entry["iyr"], 2010, 2020) ||
				!checkNumericField(entry["eyr"], 2020, 2030) {
				continue
			}
			...
			if unit == "cm" || unit == "in" {
				if unit == "cm" && !checkNumericField(measurement, 150, 193) {
					continue
				}
				if unit == "in" && !checkNumericField(measurement, 59, 76) {
					continue
				}
			} else {
				continue
