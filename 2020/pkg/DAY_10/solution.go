...
and count the joltage differences between the charging outlet, the adapters, and your device. What is the number
of 1-jolt differences multiplied by the number of 3-jolt differences?
*/
func prepData(path string) []int {
...
func partOne(data []int) int {
	...
	for _, adapter := range data {
		//fmt.Printf("adapter: %d, prevJolts: %d\n", adapter, prevJolts)
		if adapter == prevJolts+1 {
			onejoltsDif++
		}
		if adapter == prevJolts+3 {
			threeJoltsDif++
		}
		prevJolts = adapter
	}
	return onejoltsDif * threeJoltsDif
}
...
Thoughts:
	...
	First thoughts would be to use some sort of backtracking/recursion, however taking note of the hint that
	there must be 'more than a TRILLION' adpater permutations we might get into a spot of trouble with timeouts.
	Time to dust off the dynamic programming stuff (had to look some of this up).
*/

func partTwo(data []int) int64 {
	cache := make(map[int]int64)
	data = append([]int{0}, data...)
	return partTwoHelper(data, 0, cache)
}

func partTwoHelper(data []int, i int, cache map[int]int64) int64 {
	if i == len(data)-1 {
		return 1
	}

	if count, ok := cache[i]; ok {
		return count
	}
	...
	for j := i + 1; j < len(data); j++ {
		...
		res := partTwoHelper(data, j, cache)
		total += res
		cache[j] = res
	}
	return total
