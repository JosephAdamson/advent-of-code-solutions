...
Thoughts (naive?):
	...
	For each interation after the preamble:
		- sliding window map; after each iteration remove the element at the lower bound of the preamble
		and add the one located at the new upper bound.
		- iterate over set, take an element away and query the set with the difference O(1) at most O(set size)
		- if we get one that doesn't fit return value
		- O(n * permable query loops)
*/
func partOne(data []int, preamble int) int {
	// pointers for our preamble window
	a := 0
	b := preamble
	n := len(data)
	preambleSet := map[int]bool{}
	...
	for b < n {
		val := data[b]
		//log.Printf("a: %d, b %d, val: %d", a, b, val)
		for i, num := range data[a:b] {
			dif := val - num
			_, ok := preambleSet[dif]

			if ok && (num != dif) {
				//log.Printf("%d + %d = %d", num, dif, val)
				break
			}
		...
		delete(preambleSet, data[a])
		a++
		b++
		preambleSet[data[b]] = true
	}
	//  sequence is all clear
	return -1
}
...
func partTwo(data []int, preamble int) int {
	...
	a := 0
	b := 1
	sum := big.NewInt(int64(data[0]))
	n := len(data)
	for b < n {
		if sum.Cmp(invalidNum) == 0 {
			break
		}

		if sum.Cmp(invalidNum) == 1 {
			sum.Sub(sum, big.NewInt(int64(data[a])))
			//log.Printf("sum: %d, invalidNum: %d\n", sum, invalidNum)
			a++
			if sum.Cmp(invalidNum) == 0 {
				break
			}
		} else {
			sum.Add(sum, big.NewInt(int64(data[b])))
			b++
		}
	}
	window := make([]int, (b - a))
	copy(window, data[a:b+1])
	sort.Slice(window, func(i, j int) bool {
		a := big.NewInt(int64(window[i]))
		b := big.NewInt(int64(window[j]))
		return a.Cmp(b) == -1
	})
...
func main() {
	...
	path = path + "/input_D9.txt"
	data, _ := utils.ReadInputInt(path, "\n")
	fmt.Println(partOne(data, 25))
	fmt.Println(partTwo(data, 25))
}
