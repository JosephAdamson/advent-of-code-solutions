...
func createRuleSet(path string) Ruleset {
	rawData, err := utils.ReadInputStr(path, "\n")
	if err != nil {
		panic("could not parse data")
	}
	ruleset := map[string]map[string]int{}
	pattern := regexp.MustCompile(`[.]|bags|bag|`)
	for _, rule := range rawData {
		parentAndChildren := strings.Split(rule, "contain")
		parent := pattern.ReplaceAllString(parentAndChildren[0], "")
		parent = strings.TrimSpace(parent)
		contents := strings.Split(parentAndChildren[1], ",")
		children := map[string]int{}
		for i := 0; i < len(contents); i++ {
			contents[i] = pattern.ReplaceAllString(contents[i], "")
			contents[i] = strings.TrimSpace(contents[i])
			if contents[i] == "no other" {
				children[contents[i]] = 0
				continue
			}
			tokens := strings.Split(contents[i], " ")
			count, err := strconv.Atoi(tokens[0])
			if err != nil {
				panic("failed to parse string")
			}
			children[tokens[1]+" "+tokens[2]] = count
		}
		ruleset[parent] = children
	}
	return ruleset
}
...
be valid for the outermost bag? (In other words: how many colors can, eventually, contain at least one shiny
gold bag?)
...
*/
func partOne(ruleset Ruleset, bag string) int {
	options := map[string]bool{}
	partOneHelper(ruleset, bag, options)
	return len(options)
}

func partOneHelper(ruleset Ruleset, bag string, options map[string]bool) {
	for parent, children := range ruleset {
		if _, exists := children[bag]; exists {
			if _, visited := options[parent]; !visited {
				options[parent] = true
				partOneHelper(ruleset, parent, options)
			}
		}
	}
}
...
*/
func partTwo(ruleset Ruleset, bag string) int {
	children := ruleset[bag]
	breadthSum := 0
	for child, count := range children {
		subcount := partTwo(ruleset, child)
		if subcount > 0 {
			breadthSum += (count * subcount)
		}
		breadthSum += count
...
func main() {
	...
	path = path + "/input_D7.txt"
	ruleset := createRuleSet(path)
	fmt.Println(partOne(ruleset, "shiny gold"))
	fmt.Println(partTwo(ruleset, "shiny gold"))
}
