...
Thoughts: this is basically a variation of the two sum problem (https://leetcode.com/problems/two-sum/)
*/
func partOne(data []int) int {
	// still use a map for constant access
	prevVals := map[int]int{}
...
they had left over from a past vacation. They offer you a second one if you can find three numbers in your
...
func partTwo(data []int) int {
	...
	for i, curr := range data {
		...
		for _, preVal := range keys {
			dif := totalDif - preVal
			_, ok := preVals[dif]
			if ok {
				return curr * dif * preVal
			}
		}
		preVals[curr] = i
	}
...
func main() {
	...
	data, _ := utils.ProcessIntData(path, "\n")
