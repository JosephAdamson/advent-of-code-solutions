...
func parseData(path string) (int, []int, error) {
	rawData, _ := utils.ReadInputStr(path, "\n")

	timestamp, error := strconv.Atoi(rawData[0])
	if error != nil {
		return 0, nil, fmt.Errorf("timestamp could not be parsed")
	}

	busesData := strings.Split(rawData[1], ",")
	var buses []int
	for _, bus := range busesData {
		if bus != "x" {
			value, error := strconv.Atoi(bus)
			if error != nil {
				return 0, nil, fmt.Errorf("buses could not be parsed")
			}
			buses = append(buses, value)
		}
	}
	return timestamp, buses, nil
}

func partOne(timestamp int, buses []int) int {
	n := len(buses)
	multiples := make([]int, n)
	copy(multiples, buses)

	for i := 0; i < n; i++ {
		multiples[i] = timestamp - timestamp%multiples[i]
	}

	for {
		for i := 0; i < n; i++ {
			multiples[i] = multiples[i] + buses[i]
		}
		result := math.MaxInt32
		resultIndex := -1
		for i, multiple := range multiples {
			if multiple > timestamp && multiple < result {
				result = multiple
				resultIndex = i
			}
		}
		if result < math.MaxInt32 {
			return (result - timestamp) * buses[resultIndex]
		}
	}
}
...
The only bus departures that matter are the listed bus IDs at their specific offsets from t. Those bus IDs
 can depart at other times, and other bus IDs can depart at those times. For example, in the list above,
...
func main() {
	...
	path = path + "/input_D13.txt"
	timestamp, buses, _ := parseData(path)
	fmt.Println(partOne(timestamp, buses))
}
