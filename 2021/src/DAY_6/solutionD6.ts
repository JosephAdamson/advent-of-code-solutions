...
value). The new lanternfish starts with an internal timer of 8 and does not start counting down until the next 
day.
...
- The above solution wouldn't work for a large n because of its O(n^2) time complexity where n scales
 exponentially, so we need to do some optimization. This could be done for part one also.

HINT (from the reddit): “To solve this puzzle, you only need a 9-element array.”

- well, there are nine states (including the zeroith) that a fish can be in so...
so each element could represent the amount/frequency of fish in each state at a given time?

For the initial state of the starting example: 3,4,3,1,2, this would be represented as
...
function partTwo(school: number[] | undefined) {
    if (school) {
        const schoolFrequency: bigint[]= Array(9).fill(0);
        for (let value of school) {
            schoolFrequency[value]++;
        }
        
        let babies: bigint = BigInt(0);
        for (let day = 0; day < 256; day++) {
            babies = schoolFrequency[0];
            for (let i = 1; i < schoolFrequency.length; i++) {
                schoolFrequency[i - 1] = schoolFrequency[i];
            }
            schoolFrequency[6] += babies;
            schoolFrequency[8] = babies;
        }
        let result: bigint = BigInt(0);
        for (let value of schoolFrequency) {
            result += BigInt(value);
        }
        return result;
    }
