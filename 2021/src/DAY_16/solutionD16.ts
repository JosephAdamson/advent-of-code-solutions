...
const PATH = resolve("src/DAY_16/input_D16.txt");

function readIn(path: string) {
    const dataRaw = readFileSync(path, "utf-8");
    return dataRaw;
}   

const PACKET = readIn(PATH);
/* 
...
and the next three bits encode the packet type ID. These two values are numbers; all 
numbers encoded in any packet are represented as binary with the most significant bit first. 
...
prefixed by a 1 bit except the last group, which is prefixed by a 0 bit. These groups of 
five bits immediately follow the packet header. For example, the hexadecimal string D2FE28 
becomes:
...
Thoughts:
    ...
    - Read through the string in a recursive/iterative manner skipping the chunks that 
    don't contain the header info. That's all I got.
    
    Optimization?
    - converting this large hex string into binary alone is going to take 
    a lot of memory. We will probably have to mitigate this usage in some way?

    - remember EACH hex character is a byte so the the resulting hex to binary string n * 4
*/
...
function binaryfy(packet: string) {
    let binRep = "";
...
// for part two
function perfromOp(id: number, literals: number[]): number {
    switch (id) {
        case 0:
            return literals.reduce((acc, curr) => acc + curr);
        case 1:
            return literals.reduce((acc, curr) => acc * curr);
        case 2:
            return Math.min(...literals);
        case 3:
            return Math.max(...literals);
        case 5:
...
function parsePackets(packet: string, start: number, packetVersions: number[]): number[] {
    const version = parseInt(packet.slice(start, start + 3), 2);
    packetVersions.push(version);
    start += 3;
    const id = parseInt(packet.slice(start, start + 3), 2);
    start += 3;

    if (id === 4) {
        // seek to the end of the literal packet.
        return parseLiteralPacket(packet, start);
    }
    
    const lengthId = parseInt(packet[start]);
    start++;
    const literals = [];
    if (lengthId === 0) {
        // 15 bits contains the length, we don't know how many sub-packets
        // are contained within but we have an established bounds for packet
        // seeking.
        const subLen = parseInt(packet.slice(start, start + 15), 2);
        start += 15;
        const endOfPacket = start + subLen;
        while(start < endOfPacket) {
            const [newStart, literal] = parsePackets(packet, start, packetVersions);
            start = newStart;
            literals.push(literal);
        }
    } else {
        // 11 bits contains the number of subpackets there are and therefore how many times
        // we have to seek,
        const packetNum = parseInt(packet.slice(start, start + 11), 2);
        start += 11; 
    ...
    const literalRes = perfromOp(id, literals);
    return [start, literalRes];  
}

function parseLiteralPacket(packet: string, start: number): number[] {
    let chunk = packet.slice(start, start + 5);
    let literalBin = chunk.slice(1);
    while (chunk[0] != "0") {
        start +=5;
        chunk = packet.slice(start, start + 5);
        literalBin += chunk.slice(1);
    }
    const literal = parseInt(literalBin, 2);
    start += 5;
    return [start, literal];
}

function partOne(packet: string){
    const binaryPacket = binaryfy(packet);
    // memoized version sum (stored in array for access across call stack)
    const packetVersions: number[] = [];
    parsePackets(binaryPacket, 0, packetVersions);
    return packetVersions.reduce((acc, curr) => acc + curr);
}

console.log(partOne(PACKET));
...
values of their sub-packets. If they only have a single sub-packet, their value is the value of the 
sub-packet.
Packets with type ID 2 are minimum packets - their value is the minimum of the values of their 
sub-packets.
Packets with type ID 3 are maximum packets - their value is the maximum of the values of their s
ub-packets.
Packets with type ID 5 are greater than packets - their value is 1 if the value of the first sub-packet 
is greater than the value of the second sub-packet; otherwise, their value is 0. These packets always 
...
function partTwo(packet: string) {
    const binaryPacket = binaryfy(packet);
    const [end, result] = parsePackets(binaryPacket, 0, []);
    return result;
}

console.log(partTwo(PACKET));
