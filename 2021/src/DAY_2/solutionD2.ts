...
function partOne(data: string[] | undefined) {
    ...
    if (data) {
        ...
        for (let command of data) {
            const [cmd, inc] = command.split(" ");
            switch (cmd) {
                case "up": dPos -= parseInt(inc);
                break;

                case "down": dPos += parseInt(inc);
                break;

                case "forward": hPos += parseInt(inc);
                break;
...
function partTwo(data: string[] | undefined) {
    if (data) {
        ...
        for (let command of data) {
            const [cmd, inc] = command.split(" ");
            switch (cmd) {
                case "down": aim += parseInt(inc);
                break;

                case "up": aim -= parseInt(inc);
                break;
