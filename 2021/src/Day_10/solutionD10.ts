...
const PATH = resolve("src/DAY_10/input_D10.txt");

let data: string[] | undefined = readInDataStr(PATH);
...
on each line, and chunks contain zero or more other chunks. Adjacent chunks are not separated by any delimiter; 
...
Similar to finding 'symmetical parenthesis' problem I have seen on various sites before.
Use a stack based solution to determine whether chunk sequence is incomplete or unfinished

- we iterate through each sequence
    • compare the previous chunk to any ], }, ), > chunks
        * if the chunk matches we can disgard both and leave the stack
        * if we finish iterating through the sequence and we don't have an empty stack we have got 
        an unfinished sequence.
        * if a chunk doesn't match we have and error and we disregard the stack.
*/

const errorScoreTable: Map<string, number> = new Map();
errorScoreTable.set(")", 3);
errorScoreTable.set("]", 57);
errorScoreTable.set("}", 1197);
errorScoreTable.set(">", 25137);


interface chunkResults {
    errorChunks: string[],
    unfinishedChunks: string[][]
}


function readChunks(data: string[]) {
    const errorChunks: string[] = [];
    const unfinishedChunks: string[][] = [];

    for (let chunkSequence of data) {
        const chunks = chunkSequence.split("");
        ...
        // if the stack is NOT empty at the end of the iteration we have unfinished chunk sequence.
        if (stack.length !== 0) {
            stack.shift();
            unfinishedChunks.push(stack);
        }
    }
    const results: chunkResults = {
        errorChunks: errorChunks,
        unfinishedChunks: unfinishedChunks
    }
    return results;
}
...
function partOne(data: string[] | undefined) {
    if (data) {
        const results = readChunks(data);
        const errors = results.errorChunks;
        let syntaxErrorScore = 0;
        for (let chunk of errors) {
            const score: number | undefined = errorScoreTable.get(chunk);
            if (score) {
...
total score by 5 and then increase the total score by the point value given for the character in the following 
table:
...
the middle score? 
*/
const completionTable: Map<string, number> = new Map();
completionTable.set("(", 1);
completionTable.set("[", 2);
completionTable.set("{", 3);
completionTable.set("<", 4);


function partTwo(data: string[] | undefined) {
    if (data) {
        const results = readChunks(data);
        const unfinished = results.unfinishedChunks;
        //console.log(unfinished);
        const scores: number[] = [];
        for (let sequence of unfinished) {
            let sequenceScore = 0;
            for (let i = sequence.length - 1; i >= 0; i--) {
                const chunkScore = completionTable.get(sequence[i]);
                if (chunkScore) {
                    sequenceScore = sequenceScore * 5 + chunkScore;
                }
            }
