...
performs a sonar sweep of the nearby sea floor. On a small screen, the sonar sweep report 
(your puzzle input) appears: each line is a measurement of the sea floor depth as the sweep looks 
...
function partTwo(data: number[] | undefined) {
    if (data) {
        ...
        let j = 2;
        while (j < data.length) {
            let newWindow = prevWindow - data[i++] + data[++j];
