...
4846848554
5283751526
The energy level of each octopus is a value between 0 and 9. Here, the top-left octopus has an energy level of 
5, the bottom-right one has an energy level of 6, and so on.
...
Thoughts:
    Brute force (not ideal):
        - Model data as 2d martix
        - Keep track of octopi that have already flashed for each step (separate martix)
        - Initial pass through data to increment each value (O(n))
        - Second pass that finds elemnts equal to 10
        - once we find a 10 we increment neighbours, reset value and register the location of the flash
        (this octopus is now on cool down).
...
function runOctopusSim(data: number[][], steps: number, flashCount: number[]) {
    ...
    let flashTable = [];
        for (let step = 0; step < steps; step++) {
            ...
            for (let row = 0; row < n; row++) {
                flashTable.push(Array(m).fill(0));
            }
            ...
            while (p < n - 1) {
                while(q < m - 1) {
                    if (data[p][q] >= 10 && flashTable[p][q] !== -1) {
                        checkflashed(p, q - 1, flashTable, data);
                        checkflashed(p - 1, q - 1, flashTable, data);
                        checkflashed(p - 1, q, flashTable, data);
                        checkflashed(p - 1, q + 1, flashTable, data);
                        checkflashed(p, q + 1, flashTable, data);
                        checkflashed(p + 1, q + 1, flashTable, data);
                        checkflashed(p + 1, q, flashTable, data);
...
function partOne(data: number[][] | undefined) {
    if (data) {
        const paddedData = padMatrix(data, 11);
        const flashCount = [0];
        runOctopusSim(paddedData, 100, flashCount);
        return flashCount[0];
    }
}
...
Thoughts:
- Brute force; run sim (alter data state in place) and check the state of the data after each step.
*/
function partTwo(data: number[][] | undefined) {
    if (data) {
        ...
        let stepCount = 0;
        while (true) {
            runOctopusSim(paddedData, 1, [0]);
            stepCount++;
            let sum = 0;
            for (let i = 1; i < paddedData.length - 1; i++) {
                for (let j = 1; j < paddedData[0].length - 1; j++) {
                    sum += paddedData[i][j];
                }
            }
            if (sum === 0) {
