...
As you walk, the Elf shows you a small bag and some cubes which are either red, green, or blue. Each time
you play this game, he will hide a secret number of cubes of each color in the bag, and your goal is to
...
You play several games and record the information from each game (your puzzle input). Each game is listed
 with its ID number (like the 11 in Game 11: ...) followed by a semicolon-separated list of subsets of cubes
...
Outer:
	for i, line := range lines {
		idSets := strings.Split(line, ":")
		subsets := strings.Split(idSets[1], ";")

		for _, subset := range subsets {
			amounts := strings.Split(subset, ",")
			for _, pair := range amounts {
				pair = strings.TrimSpace(pair)
				numColor := strings.Split(pair, " ")
				value, _ := strconv.Atoi(numColor[0])

				if numColor[1] == "red" && value > 12 {
					continue Outer
				}
				if numColor[1] == "green" && value > 13 {
					continue Outer
				}
				if numColor[1] == "blue" && value > 14 {
					continue Outer
				}
			}
		}
	...
	for _, line := range lines {
		...
		for _, subset := range subsets {
			...
			for _, pair := range amounts {
				...
				if numColor[1] == "red" && value > maxRed {
					maxRed = value
				}
				if numColor[1] == "green" && value > maxGreen {
					maxGreen = value
				}
				if numColor[1] == "blue" && value > maxBlue {
					maxBlue = value
				}
			}
		}
